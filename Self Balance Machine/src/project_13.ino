...
//   lcd.print(Gyro_x);
  ...
  if(Serial.available()) {
    ...
    switch(val) {            //switch statement
      case 'F': 
        front = 250; 
        break;       //if val equals to F，front=250，balance robot goes front.
      case 'B': 
        back = -250; 
        break;       //go back
      case 'L': 
      ...
      case 'D': 
        Serial.print(angle);
        break;    //when receiving ‘D’，send value of angle to APP
    }
  }
...
void DSzhongduan() {
  ...
  if(lcd_count >= 200) {    //5*200=1000，1000ms entering once LCD display
    lcd_count = 0;  //Clear
    lcd.clear();
    lcd.setCursor(0,0);
    lcd.print("angle = ");
